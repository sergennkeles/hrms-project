...
//			+ " From JobAdvertisement j Inner Join j.city c ")
	@Query("select new Hrms.hrmsProject.entities.dtos.JobAdvertisementDto(j.description,j.minSalary,j.maxSalary,j.createdDate,j.deadLineDate,j.openPositionNumber,jo.jobsName, e.companyName,c.cityName) "
			+ "from JobAdvertisement j inner join j.employer e inner join j.job jo inner join j.city c")
