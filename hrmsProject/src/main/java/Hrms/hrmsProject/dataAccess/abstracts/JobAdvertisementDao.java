...
public interface JobAdvertisementDao extends JpaRepository<JobAdvertisement, Integer> {
	...
	List<JobAdvertisement> findByIsActiveFalse();
	
	List<JobAdvertisement> findByWorkingTypeId(int id);
	
	List<JobAdvertisement> findByWorkingTimeId(int id);
	
	List<JobAdvertisement> findByCityId(int id);
	
	List<JobAdvertisement> findByJobId(int id);
...
//			+ " From JobAdvertisement j Inner Join j.city c ")
	@Query("select new Hrms.hrmsProject.entities.dtos.JobAdvertisementDto(j.id, j.description,j.minSalary,j.maxSalary,wti.workingTime,wty.workingType,j.createdDate,j.deadLineDate,j.openPositionNumber,jo.jobsName, e.companyName,c.cityName) "
